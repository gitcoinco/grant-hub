...
async function main() {
  ...
  await prompt("do you want to deploy the Direct Donation Contract?");
  console.log("deploying...");

  const directDonationFactory = await ethers.getContractFactory("DirectDonationImplementation", account);
  const instance = await upgrades.deployProxy(directDonationFactory, []);
  ...
  console.log("Direct Donation Contract deployed to:", instance.address);
}
