...
async function main() {
  ...
  await prompt("do you want to upgrade the Direct Donation Contract to V2?");
  console.log("deploying...");

  const directDonationFactoryV2 = await ethers.getContractFactory("DirectDonationImplementationV2", account);
  const upgraded = await upgrades.upgradeProxy(PROXY_ADDRESS, directDonationFactoryV2);
  console.log("tx hash", upgraded.deployTransaction.hash);
  await upgraded.deployed();

  console.log("Direct Donation Contract deployed to:", upgraded.address);
}
