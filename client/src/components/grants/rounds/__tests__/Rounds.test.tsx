  ...
  describe("when the data is loaded", () => {
    ...
    test.only("should show the active rounds", async () => {
      const round1 = buildRound({
        address: addressFrom(1), applicationsEndTime: 1663751954, // you need to set the correct application and round times which fits into your if statements
      });
      ...
      const applications = [];
      applications.push(
        buildProjectApplication({ roundID: addressFrom(1), status: "APPROVED" }) // set the status directly here, saves some pain
      );
      applications.push(
